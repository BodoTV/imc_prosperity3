...
class HelperFunctions: 
    @staticmethod
    def get_price_history(state: TradingState, product: str) -> list[float]:
        """Get the list of buy prices for a given product."""
        return list(state.order_depths[product].buy_orders.keys())
...
class MarketMakingStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        # # Only adjust order book imbalance if default_price is not None
        if default_price is not None:
            self.price_history.append(default_price)

            # Track order book imbalance
            order_book_imbalance = self.get_order_book_imbalance(state)

            # Adjust pricing based on order book imbalance
            if order_book_imbalance > 0.2:
                max_buy_price = default_price + 1  # Increase bid price if imbalance suggests upward movement
            elif order_book_imbalance < -0.2:
                min_sell_price = default_price - 1  # Lower ask price if imbalance suggests downward movement
            else:
                max_buy_price = default_price  # Maintain default buy price if imbalance is small
                min_sell_price = default_price  # Maintain default sell price if imbalance is small
        else:
            # If default_price is None, do not modify the order book imbalance pricing
            max_buy_price = None
            min_sell_price = None
        ...
        # Adjust pricing based on order book imbalance--> all values hardcoded until now. lets see
        if order_book_imbalance > 0.2:
            max_buy_price += 1  # Increase bid price if imbalance suggests upward movement
        elif order_book_imbalance < -0.2:
            min_sell_price -= 1  # Lower ask price if imbalance suggests downward movement
    ...
    def get_order_book_imbalance(self, state: TradingState) -> float:
        total_bid_vol = sum(state.order_depths[self.product].buy_orders.values())
        total_ask_vol = sum(state.order_depths[self.product].sell_orders.values())
        return (total_bid_vol - total_ask_vol) / (total_bid_vol + total_ask_vol) if (total_bid_vol + total_ask_vol) > 0 else 0
