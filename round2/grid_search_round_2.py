...
def run_gridsearch(param_grid, 
    ...
    if independent_products:
        for product in param_grid.keys():
            ...
            for combination in param_combinations:
                ...
                for argument in product_param_grid.keys():
                    print("     ",  argument, " = " , params[argument])
...
def run(trader : Trader,
    ...
    for product in products:
        ...
        if pnls:
            ...
            final_pnl = 0  # no trades or data for this product
            
        log["total_pnl"] += final_pnl
        log[f"total_{product}_pnl"] += final_pnl

        print(product + "_final_pnl", final_pnl)

    print("total_pnl = ", log["total_pnl"])

    return log
    
#this defines how the grid search will be run [default parameter, +/- grid size, increment]
gridsearch_args = {
            "PICNIC_BASKET1" : {"long_thresh" : [150, 120, 30],
                                  "short_thresh": [100, 50, 10]}
        }
...
default_args = {
            ...
            "KELP" : {"history_size": 10, 
                      "soft_liquidation_tresh" : 0.5},
            "SQUID_INK" : {"beta_reversion" : 0.369,
                                  "volume_threshold": 12},
            "PICNIC_BASKET1" : {"long_thresh" : 50,
                                  "short_thresh": 100},
            "PICNIC_BASKET2" : {"long_thresh" : 50,
                                  "short_thresh": 100},
             "CROISSANTS" : {"long_thresh" : 50,
                                  "short_thresh": 100},
            "DJEMBES" : {"long_thresh" : 50,
                                  "short_thresh": 100},
            "JAMS" : {"long_thresh" : 50,
                                  "short_thresh": 100},
        }
...
param_grid = {product : {argument : 
                         ...
                         for argument in gridsearch_args[product]} for product in gridsearch_args.keys()}
