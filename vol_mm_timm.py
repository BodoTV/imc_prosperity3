...
class SquidInkStrategy(MarketMakingStrategy):
    ...
    def get_default_price(self, state) -> float:
        ...
        #mean reversion with running autocorrelation
        self.mid_price_history.append(mm_mid_price)

        if len(self.mid_price_history) >= 3:
            returns = [(self.mid_price_history[i] - self.mid_price_history[i - 1]) / self.mid_price_history[i - 1]
                    for i in range(1, len(self.mid_price_history))]
            x = np.array(returns[:-1])
            y = np.array(returns[1:])
            corr = np.corrcoef(x, y)[0, 1]
            corr = corr if np.isfinite(corr) else -0.369 #because corrcoef returns nans if y,x too similar
        else:
            corr = -0.369
        ...
        if self.last_mm_mid_price != None:
            ...
            pred_returns = ( 
                last_returns * corr #this tries to predict how much 
            )
...
class KelpStrategy(MarketMakingStrategy):
    ...
    def get_default_price(self, state: TradingState) -> int:
        ...
        if len(self.mid_price_history) >= 3:
            ...
            y = np.array(returns[1:])
            corr = np.corrcoef(x, y)[0, 1]
            corr = corr if np.isfinite(corr) else -0.5 #because corrcoef returns nans if y,x too similar
        else:   
            corr = -0.5
